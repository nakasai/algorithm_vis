...
import { Play, Pause, StepBack, StepForward, RotateCcw, FastForward, Edit, Check, ChevronDown, ChevronRight } from 'lucide-react';

const KnapsackVisualizerEnhanced = () => {
  // 初期データ
  const defaultItems = [
  ...
  useEffect(() => {
    if (step >= totalSteps) {
      try {
        // 計算完了
        const finalDpTable = Array(N + 1).fill().map(() => Array(W + 1).fill(0));
      ...
      try {
        ...
        // DPテーブルの計算
        const newDpTable = Array(N + 1).fill().map(() => Array(W + 1).fill(0));
  ...
  const initializeDP = () => {
    try {
      ...
      const newDpTable = Array(rowCount).fill().map(() => Array(colCount).fill(0));
      setDpTable(newDpTable);
      setSelectedItems([]);
      setCurrentI(0);
      setCurrentW(0);
    ...
    if (isComplete) {
      return (
        <div className="space-y-2">
          <p>計算完了！最適な総価値: <span className="font-bold text-xl">{dpTable[N][W]}</span></p>
          <p>選択されたアイテム:</p>
...
<span class="token punctuation">}</span>`}} /></pre>
            ...
            <div className="mt-3">
              ...
              <ol className="list-decimal pl-5 text-sm mt-1 space-y-1">
                {selectedItems.length > 0 && dpTable && dpTable.length > 0 && (
                  <>
                    <li>
                      最大重量 W = {W}、最適値 = dp[{N}][{W}] = {dpTable[N] && dpTable[N][W]}
                    </li>
                    {Array.from({ length: N }, (_, idx) => {
                      ...
                      return (
                        <li key={i} className={isSelected ? "font-medium" : ""}>
                          i={i}（{item.name}）: dp[{i}][{remainingW}] = {dpTable[i] && dpTable[i][remainingW]}, 
                          dp[{i-1}][{remainingW}] = {dpTable[i-1] && dpTable[i-1][remainingW]}
                          {isSelected ? (
                            <> 
            ...
            <div className="mt-3">
              ...
              <ul className="list-disc pl-5 text-sm mt-1">
                <li><span className="font-bold text-red-600">赤枠の太いセル (dp[N][W])</span>: 最終的な最適値 {dpTable[N] && dpTable[N][W]}</li>
                  ...
                  <ul className="list-disc pl-5 mt-1">
                    {(() => {
                      ...
                      for (let i = 0; i < selectedItems.length; i++) {
                        ...
                        cells.push(
                          <li key={i}>
                            dp[{itemIndex}][{remainingWeight}] = {dpTable[itemIndex] && dpTable[itemIndex][remainingWeight]}
                          </li>
                        );
                      ...
                      for (let i = 0; i < selectedItems.length; i++) {
                        ...
                        cells.push(
                          <li key={i}>
                            ...
                            {dpTable[itemIndex-1] && dpTable[itemIndex-1][remainingWeight-item.weight]} + {item.value} =&nbsp;
                            {(dpTable[itemIndex-1] && dpTable[itemIndex-1][remainingWeight-item.weight] || 0) + item.value}
                          </li>
                        );
...
int main() {
    ...
    const highlightCodeLine = (line) => {
      ...
      const leadingSpaces = mainCode.match(/^(\s*)/)[0];
  ...
  const renderDPTable = () => {
    ...
    // テーブルのスクロール領域を設定
    const tableWidth = Math.min(1000, (W + 2) * 60);
